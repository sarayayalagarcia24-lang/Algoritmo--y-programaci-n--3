...
Termino* insertarTermino(Termino* cabeza, float coef, int exp) {
    ...
    while (actual->siguiente != nullptr && actual->siguiente->exponente > exp) {
        actual = actual->siguiente;
    }
    if (actual->siguiente != nullptr && actual->siguiente->exponente == exp) {
...
Termino* sumarPolinomios(Termino* p1, Termino* p2) {
    ...
    Termino* actual = p1;
    while (actual != nullptr) {
        resultado = insertarTermino(resultado, actual->coeficiente, actual->exponente);
        actual = actual->siguiente;
    }
    actual = p2;
    while (actual != nullptr) {
...
int main() {
    Termino* p1 = nullptr;
    p1 = insertarTermino(p1, 3, 2);
    p1 = insertarTermino(p1, 2, 1);
    p1 = insertarTermino(p1, 5, 0);

    Termino* p2 = nullptr;
    p2 = insertarTermino(p2, 1, 2);
    p2 = insertarTermino(p2, 4, 0);

    separador("POLINOMIO 1");
    imprimirPolinomio(p1);
    separador("POLINOMIO 2");
    ...
    Termino* mult = multiplicarPolinomios(p1, p2);
    imprimirPolinomio(mult);

    separador("DERIVADA DE P1");
    Termino* deriv = derivarPolinomio(p1);
    imprimirPolinomio(deriv);

    cout << "\nP1 evaluado en x=2: " << evaluarPolinomio(p1, 2) << endl;
    cout << "Grado de P1: " << gradoPolinomio(p1) << endl;
    cout << "Terminos de P1: " << contarTerminos(p1) << endl;

    destruirPolinomio(p1);
    destruirPolinomio(p2);
    destruirPolinomio(suma);
    destruirPolinomio(mult);
    destruirPolinomio(deriv);
